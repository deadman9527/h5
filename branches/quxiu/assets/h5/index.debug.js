...
;(function ($, Global) {
    ...
    Global.formatTitleLink = function (link, title,alias) {
        /*if (!title) {
         link = link.replace('http://h5.quxiu.me/', '');
         return link;
         }*/
        var url = new lib.httpurl(link);
        url.params.title = title ? title : "";
        var oldUrl = url.toString().replace('http://h5.quxiu.me/', '');
    ...
    var main = {
        init: function (opts) {
            ...
            $('#downapp').on('click', function(e) {
                var schema = "http://h5.quxiu.me/index.html";
                var params = true;
                var point = true;
        ...
        renderFlash:function(){
            ...
            $.ajax({
                url: "http://act.quxiu.me/data/indexflash.json",
                data: {},
                dataType: 'json',
                //jsonpCallback: 'jsonp',
        ...
        renderMain: function (filter) {
            ...
            lib.api.get({
                ...
                success: function (data) {
                    ...
                    // data.cate && that.renderCate(data.cate);
                    // data.guide && that.renderTabheader(data.guide);
                    that.categoryId = data.cateid
                    data.cateid && that.loadRecommend(false,data.cate);
...
//                console.log(e.selectIndex);
        ...
        renderRush: function (data) {
            ...
            var swiper = new Swiper('#rush-items', {
                slidesPerView: 'auto',
        ...
        loadRecommend: function (reset, categoryList) {
            var that = this;
            if(reset){
                that.page = 1;
            }else{
                if(that.isAjaxRecommends) {
                    return;
                }
            }
            that.isAjaxRecommends = true;
            lib.api.get({
                api: {
                    c: "main",
                    a: "get_nav_goods",
                    nav_id: that.categoryId||'',
                    is_index: 1
                },
                data: {
                    p: this.page
                ...
                success: function(data){
                    ...
                    data.list && that.renderRecommend(data.list,categoryList,reset);
        ...
        renderRecommend: function (itemList,categoryList,reset) {
            var that = this;
            var existNumber = $('.recommend-item').length - 1;
            if(reset){
            ...
            $('.info-tip').hide();
            var $target = $('#recommendList');
            var tpl = $('#tpl-recommend').html();
            ...
            var recommend = itemList;

            // for(var i=0;i<recommend.length;i++){
            //     var item = recommend[i];
            //     if(item && item.time_end){
            //         //that.time = false;
            //         //console.log(item.time_end);
            //         var time = parseInt(item.surplusTime);
            ...
            //         //var time = Date.parse(item.time_end.replace(/-/g, '/')) - Date.now();
            //         var recommendId = Number((((item.targetUrl).match(/recommendId=\d+/)[0]).match(/\d+/))[0]);
            //         //console.log(time);
            ...
            //         that.renderTime(count,i,recommendId);
            //     }
            // }
...
//                    "targetUrl": "http://h5.quxiu.me/list.html?brandId=1354"
//                }
            ]
        });
    })
