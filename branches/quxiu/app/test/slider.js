...
;(function(win, lib){
    ...
    var TouchSlider = function(container,options){
        ...
        $.extend(this,{
            ...
            steps : 0,  //步长，每次滑动的距离
            left : 0,  //panel初始的x坐标
            direction: 'x',//滑动方向，默认为x,横向滑动
            special: false,//滑动方向，默认为x,横向滑动
    ...
    $.extend(TouchSlider.prototype,{
        ...
        init : function(){
            ...
            if(this.direction === 'x'){
            	...
            	this.steps = this.steps || wrap.height();  //滑动步长，默认wrap的宽度
            }
            panels.each(function(n,item){
				if(this.direction === 'x'){
                	allWidth += item.offsetWidth;
				}else{
                	allWidth += item.offsetHeight;
				}
            });
            ...
            // 循环
                ...
                if(this.direction === 'x'){
					...
					lastp.style.cssText += 'position:relative;top:'+(-this.steps*(len+2))+'px;';
					allWidth += panels[0].offsetHeight;
					allWidth += panels[len-1].offsetHeight;
                }
            }
            if(this.direction === 'x'){
            	panel.css('width',allWidth);
            }else{
            	panel.css('height',allWidth);
            }
        ...
        getImg : function(obj){  //加载图片
            ...
            //lazyload
            obj.find(".tms-itemlist").find("img").each(function(){
        ...
        move : function(e){
            ...
            var et = e.touches[0],
            disX = this._disX = this.direction === 'x' ? et.pageX - this._coord.x : et.pageY - this._coord.y,
            initLeft = this.left,
            tmleft;
            if(typeof this._movestart == 'undefined'){  //第一次执行touchmove
            	if(this.direction === 'x'){
					...
					this._movestart = !!(this._movestart || Math.abs(disX) < Math.abs(et.pageX - this._coord.x));
            	}
            }
        ...
        setCoord : function(obj,x){
        	if(this.direction === 'x'){
            	...
            	this.useTransform && obj.css("-webkit-transform",gv1 + '0px,' + x + 'px' + gv2) || obj.css("top",x);
        	}
        },
