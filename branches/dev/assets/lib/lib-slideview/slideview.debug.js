 ...
 *
 **/
(function (global, lib, undef) {
    "use strict";
    var RATEOFSNAPTHRESHOLD = 0.15,
        //导致触发翻页的卡片拖拽距离的比率
        PAGEFLIPTIME = 300,
        //翻页动画标准时长
        MAXPAGEFLIPTIME = 700,
    ...
    SlideView = (function() {
        ...
        function SlideView(el, options) {
            ...
            outer = this.wrapper.firstElementChild || doc.createElement('div');
            outer.classList.add('slider-outer');
            outer.style.cssText += 'width:100%;height:100%';
            this.outer = outer;

            slider = outer.firstElementChild || doc.createElement('ul');
            slider.classList.add('slider-wrap');
            ...
            this.items = [];
            if (!options.items) {
                options.items = slice.call(this.slider.children);
            }

            this.slider.innerHTML = '';
            for (items = options.items, i = 0, len = items.length; i < len; i += 1) {
                this.__createItem(items[i]);
            }
            ...
            compatibility.addEvent(this.wrapper, 'touchstart', this);
        ...
        SlideView.prototype = {
            ...
            onFlip: function(fn) {
                compatibility.addEvent(this.wrapper, 'slideview-flip', fn);
                this.customEvents.push(['flip', fn]);
            },
            onMoveOut: function(fn) {
                compatibility.addEvent(this.wrapper, 'slideview-moveout', fn, false);
                this.customEvents.push(['moveout', fn]);
            },
            onMoveIn: function(fn) {
                compatibility.addEvent(this.wrapper, 'slideview-movein', fn, false);
                this.customEvents.push(['movein', fn]);
            },
            onTouchStart: function(fn) {
                compatibility.addEvent(this.wrapper, 'slideview-touchstart', fn, false);
                this.customEvents.push(['touchstart', fn]);
            },
            destroy: function() {
                while (this.customEvents.length) {
                    compatibility.removeEvent(this.wrapper, 'slideview-' + this.customEvents[0][0], this.customEvents[0][1], false);
                    this.customEvents.shift();
                }
                compatibility.removeEvent(global, 'resize', this);
                compatibility.removeEvent(this.wrapper, 'touchstart', this);
                compatibility.removeEvent(this.wrapper, 'touchmove', this);
                compatibility.removeEvent(this.wrapper, 'touchend', this);
                compatibility.removeEvent(this.slider, 'transitionend', this);
            },
            __createItem: function(item) {
            ...
            slideTo: function(index) {
                if (this.items[index]) {
                    this.slider.style[cs.transitionDuration] = Math.min(MAXPAGEFLIPTIME, Math.abs(this.currentPage - index) * this.options.duration) + 'ms';
            ...
            __refreshSize: function() {
                ...
                this.maxX = -this.items.length * this.pageWidth + this.wrapperWidth;
                this.maxY = -this.items.length * this.pageHeight + this.wrapperHeight;
                ...
                this.snapThreshold = this.options.snapThreshold === null ? Math.round(this.pageSize * RATEOFSNAPTHRESHOLD) : /%/.test(this.options.snapThreshold) ? Math.round(this.pageSize * this.options.snapThreshold.replace('%', '') / 100) : this.options.snapThreshold;
            },
            __resize: function(e) {
                this.__refreshSize();
            ...
            __move: function(e) {
                ...
                //方向锁锁定，之后touchmove的方向即使不符合组件方向，也不会关闭
                this.directionLocked = true;

                if (!this.options.loop && (newPos > 0 || newPos < this.maxPos)) {
                    //已是最终且不支持loop时，减速拖拽
                    newPos = this.pos + (this.options.direction === 'h' ? deltaX : deltaY) / SLOWRATEOFDRAWOUT;
                }
            ...
            __end: function(e) {
                ...
                if (!this.options.loop && (this.pos > 0 || this.pos < this.maxPos)) {
                    if (!this.options.noBounceEasing) {
                        this.slider.style[cs.transitionDuration] = DRAWOUTBOUNCETIME + 'ms';
                    }
            ...
            __loadLazy: function() {
                ...
                for (; i < len; i += 1) {
                    if(lazyImgs[i].classList.contains(this.options.lazy)){
                        lazyImgs[i].src = lazyImgs[i].getAttribute('dataimg');
                        lazyImgs[i].removeAttribute('dataimg');
                        lazyImgs[i].classList.remove(this.options.lazy);
                    }
                }
            },
            ...
            __checkPosition: function() {
                ...
                if (this.currentPage !== currentPage) {
                    ...
                    if (activeItem) {
                        activeItem.classList.remove('slideview-active');
                    }
                ...
                activeItem = this.items[this.currentPage];
                if (activeItem) {
                    activeItem.classList.add('slideview-active');
                }
            },
    ...
    lib.slideview = function(el, options) {
