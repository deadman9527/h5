...
class QC extends Oauth{
    ...
    private function _applyAPI($arr, $argsList, $baseUrl, $method){
		...
		//print_r($keysArr);
		if($keysArr['access_token']==null || $keysArr['access_token']=="null"){
			$keysArr['access_token']=$this->qq_callback();
		}
		if($keysArr['openid']==null || $keysArr['openid']=="null"){
			$keysArr['openid']=$this->get_openid();
		}
    ...
    public function __call($name,$arg){
        ...
        if($responseArr['ret'] == 0){
			//print_r($this->keysArr);
			$r_arr = array(
				"response" => $response,
				"openid" => $this->keysArr['openid'],
			);
            return $r_arr;
            //return $responseArr;
        }else{
