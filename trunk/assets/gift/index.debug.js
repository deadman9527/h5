			...
			self.imglist.each(function (index, item) {
				if (item && self.inviewport($(item))) {
					...
					img.onerror = function () {
						self.imglist[index] = null;
						$item.removeClass('lazy').removeAttr('dataimg').css('background-image', "url(http://h5.ve.cn/assets/img/cart_img_black_640x253.png)");
						img.onload = img.onerror = null;
					}
...
;(function ($) {
    var Env = (function () {
        ...
        var url = isOnline ? 'http://www.api.ve.cn/' : 'http://test.apire.ve.cn/';
        var sign = '81cca3013245942034dbcd41b6205c36';
        ...
        init: function () {
            ...
            $.ajax({
                ...
                data: {
                    c: 'gift',
                    a: 'index',
                    pass_check_sign: Env.apiSign,
                    type: 'jsonp'
                },
                dataType: 'jsonp',
                success: function (data) {
                    if (data.code == '00000') {
//                        that.deadline = data.end_time;
                        that.deadline = 1418227200;
...
//            });
        ...
        renderCountDown: function () {
            var cd = lib.countdown({
                ...
                stringFormatter: '<\\span>dd天</\\span>:<\\span>hh</\\span>:<\\span>mm</\\span>:<\\span>ss</\\span>',
                interval: 1000,
                onUpdate: function(data){
                    document.querySelector('.countdown').innerHTML = data.stringValue;
                },
                onEnd: function(){
                    document.querySelector('#status').innerHTML = '活动已结束';
                }
            }).start();
        },
        ...
        renderError: function () {
            $('.status-bar').html('<span class="error-tip">获取活动信息失败，请重试！</span>');
        }
    };
